          ...
          <li v-for="friend in $store.state.friends" :key="friend.id">
            <ul>
              <li class="d-inline-flex">
                ...
                <div class="namelist col-lg-6 offset-xl-1">
                  <h2 v-if="friend.favourite">
                    {{ friend.name }}
                    <span><BootstrapIcon icon="star-fill" /></span>
                  </h2>
                  <h2 v-else>{{ friend.name }}</h2>

                  <button
                    class="me-1 btn btn-info border border-dark"
                    @click="visibleToggle"
                  >
                    {{ changeText }}
                  </button>
              ...
              <button
                class="me-1 btn btn-danger border border-dark"
                @click="onDelete(friend.id)"
              >
                Delete
              </button>
              ...
              <router-link
                :to="'/theresources/friendcontacts/edit/' + friend.id"
              >
                <button
...
export default {
  ...
  methods: {
    ...
    async favToggle(frndId) {
      const index = this.$store.state.friends.findIndex(
        (frnd) => frnd.id === frndId
      );
      console.log(index);
      this.$store.state.friends[index].favourite =
        !this.$store.state.friends[index].favourite;
      await axios.put(`http://localhost:3000/update/${frndId}`, {
        favourite: this.$store.state.friends[index].favourite,
      });
    },
